...
public final class SkarendalScriptEngine extends AbstractScriptEngine implements ScriptEngine, Compilable
{
   public final class LayeScript extends CompiledScript
   {
      ...
      {
         return eval(((SkarendalContext) context).root, context.getBindings(ScriptContext.ENGINE_SCOPE));
      }
   ...
   SkarendalScriptEngine(final SkarendalScriptEngineFactory factory, final ClassLoader classLoader)
   {
   ...
   {
      return ((LayeScript) compile(new StringReader(script))).eval(context);
   }
   ...
   {
      return ((LayeScript) compile(reader)).eval(context.root, bindings);
   }
   ...
   public @Override CompiledScript compile(final String script) throws ScriptException
   {
      return compile(new StringReader(script));
   }

   public @Override CompiledScript compile(final Reader reader) throws ScriptException
   {
      try
      {
