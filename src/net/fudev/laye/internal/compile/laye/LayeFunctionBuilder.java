...
public final class LayeFunctionBuilder implements FunctionBuilder
{
   ...
   {
      // TODO what do we do with FileData?? Because we preprocess, everything is one string...
      //final LayeFileData file = LayeFileData.UNKNOWN;
      final LayeValue[] k = this.k.toArray(new LayeValue[this.k.size()]);
      //@SuppressWarnings("unchecked")
      //final HashMap<LayeValue, Integer>[] jumpTables = this.jumpTables.toArray(new HashMap<LayeValue, Integer>[0]);
      ...
      return new FunctionPrototype(k, jumpTables, code, nested, upValues, lineInfo, LayeFunctionBuilder.LINE_INFOS, numParams, hasVargs, maxLocalsSize, maxStackSize, generator);
   }
   ...
   /*
   private int[] convertTempOps()
   {
      final Stack<Integer> newCode = new Stack<>();
      for (final int op : code)
      {
         newCode.push(op);
      }
      return Util.toIntArray(newCode);
   }
    */
   ...
   public void changeStackSize(final int amt)
   {
      ...
      {
         System.out.println("@line " + new Exception().getStackTrace()[1].getLineNumber() + " alloc:" + stackSize + "/" + maxStackSize);
      }
   ...
   public int addLocal(final String name, final boolean isConst)
   {
      ...
      if (local == -1)
      {
         throw new IllegalArgumentException("local variable '" + name + "' already defined in function."); // TODO different exception
      }
      return local;
   }
   ...
   private void addOp_C(final int op, final int c)
   {
      code.add(Laye.SET_C(op, c));
   }
