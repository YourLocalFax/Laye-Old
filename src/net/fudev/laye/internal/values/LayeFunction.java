...
public final class LayeFunction extends LayeValue
{
   ...
   /*
   public LayeValue resume()
   {
      if (!proto.generator)
      {
         throw new IllegalStateException("function is not a generator, cannot resume.");
      }
      return execute(lastParent, lastLocals, lastStack);
   }
   // */

   // TODO refactor some of this to move the loop into a separate method for convenience?
   private LayeValue execute(final LayeValue parent, final LayeValue[] locals, final LayeValue[] stack)
   {
      ...
      try
      {
         while (true)
         {
            ...
            {
               case Laye.OP_CLOSE: // [openUps.len() - A] CLOSE(A) for(i = #openUps; --i >= A;) openUps[i].close();
               ...
               case Laye.OP_STORE: // [1, 0] STORE(A) LOC[A] = STK[TOP - 1];
                  // This leaves the value on the top of the stack, don't forget that!
               ...
               case Laye.OP_NEW_SLOT: // [3, -2] NEW_SLOT(A) STK[TOP - 3].newSlot(STK[TOP - 2], STK[TOP - 1]); STK[TOP - 3] = STK[TOP - 1];
               {
               ...
               case Laye.OP_DEL_SLOT: // [2, -1] DEL_SLOT() STK[TOP - 2] = STK[TOP - 2].delSlot(STK[--TOP]);
               {
               ...
               case Laye.OP_GET_INDEX: // [-1] GET_INDEX() STK[TOP - 2] = STK[TOP - 2].get(STK[TOP - 1])
                  stack[top - 2] = stack[top - 2].get(stack[--top]);
                  continue;

               case Laye.OP_SET_INDEX: // [-2] SET_INDEX() STK[TOP - 3].set(STK[TOP - 2], STK[TOP - 1]); STK[TOP - 3] = STK[TOP - 1]; TOP -= 2;
               ...
               case Laye.OP_LOAD_BOOL: // [1] LOAD_BOOL(A) STK[TOP++] = if B then true; else false;
                  stack[top++] = (i >>> Laye.POS_A) != 0 ? LayeValue.TRUE : LayeValue.FALSE;
                  continue;

               case Laye.OP_LOAD_NULL: // [A] LOAD_NULL(A) (for i = 1 to A) STK[TOP++] = NULL;
               ...
               case Laye.OP_POSIT: // [0] POSIT() STK[TOP - 1] = STK[TOP - 1].posit();
                  stack[top - 1] = stack[top - 1].posit();
                  continue;

               case Laye.OP_NEGATE: // [0] NEGATE() STK[TOP - 1] = STK[TOP - 1].negate();
               ...
               case Laye.OP_COMPL: // [0] COMPL() STK[TOP - 1] = STK[TOP - 1].complement();
                  stack[top - 1] = stack[top - 1].complement();
                  continue;

               case Laye.OP_TYPEOF: // [0] TYPEOF() STK[TOP - 1] = STK[TOP - 1].typeof();
                  stack[top - 1] = stack[top - 1].typeof();
                  continue;

               case Laye.OP_PREFIX: // [0] PREFIX(A) STK[TOP - 1] = STK[TOP - 1].unaryPrefixOp(k[A]);
                  stack[top - 1] = stack[top - 1].unaryPrefixOp(k[i >>> Laye.POS_A].asstring());
                  continue;

               case Laye.OP_POSTFIX: // [0] POSTFIX(A) STK[TOP - 1] = STK[TOP - 1].unaryPostfixOp(k[A]);
                  stack[top - 1] = stack[top - 1].unaryPostfixOp(k[i >>> Laye.POS_A].asstring());
                  continue;

                  // ---------- Binary Operations ---------- //

               case Laye.OP_ADD: // [-1] ADD() STK[TOP - 2] = STK[TOP - 2].add(STK[--TOP]);
                  stack[top - 2] = stack[top - 2].add(stack[--top]);
                  continue;

               case Laye.OP_SUB: // [-1] SUB() STK[TOP - 2] = STK[TOP - 2].subtract(STK[--TOP]);
                  stack[top - 2] = stack[top - 2].subtract(stack[--top]);
                  continue;

               case Laye.OP_MUL: // [-1] MUL() STK[TOP - 2] = STK[TOP - 2].multiply(STK[--TOP]);
                  stack[top - 2] = stack[top - 2].multiply(stack[--top]);
                  continue;

               case Laye.OP_DIV: // [-1] DIV() STK[TOP - 2] = STK[TOP - 2].divide(STK[--TOP]);
                  stack[top - 2] = stack[top - 2].divide(stack[--top]);
                  continue;

               case Laye.OP_IDIV: // [-1] IDIV() STK[TOP - 2] = STK[TOP - 2].intDivide(STK[--TOP]);
                  stack[top - 2] = stack[top - 2].intDivide(stack[--top]);
                  continue;

               case Laye.OP_REM: // [-1] REM() STK[TOP - 2] = STK[TOP - 2].remainder(STK[--TOP]);
                  stack[top - 2] = stack[top - 2].remainder(stack[--top]);
                  continue;

               case Laye.OP_POW: // [-1] POW() STK[TOP - 2] = STK[TOP - 2].power(STK[--TOP]);
                  stack[top - 2] = stack[top - 2].power(stack[--top]);
                  continue;

               case Laye.OP_AND: // [-1] AND() STK[TOP - 2] = STK[TOP - 2].and(STK[--TOP]);
                  stack[top - 2] = stack[top - 2].and(stack[--top]);
                  continue;

               case Laye.OP_OR: // [-1] OR() STK[TOP - 2] = STK[TOP - 2].or(STK[--TOP]);
                  stack[top - 2] = stack[top - 2].or(stack[--top]);
                  continue;

               case Laye.OP_XOR: // [-1] XOR() STK[TOP - 2] = STK[TOP - 2].xor(STK[--TOP]);
                  stack[top - 2] = stack[top - 2].xor(stack[--top]);
                  continue;

               case Laye.OP_LSH: // [-1] LSH() STK[TOP - 2] = STK[TOP - 2].shiftLeft(STK[--TOP]);
                  stack[top - 2] = stack[top - 2].shiftLeft(stack[--top]);
                  continue;

               case Laye.OP_RSH: // [-1] RSH() STK[TOP - 2] = STK[TOP - 2].shiftRight(STK[--TOP]);
                  stack[top - 2] = stack[top - 2].shiftRight(stack[--top]);
                  continue;

               case Laye.OP_URSH: // [-1] URSH() STK[TOP - 2] = STK[TOP - 2].shiftRightUnsigned(STK[--TOP]);
                  stack[top - 2] = stack[top - 2].shiftRightUnsigned(stack[--top]);
                  continue;

               case Laye.OP_CONCAT: // [-1] CONCAT() STK[TOP - 2] = STK[TOP - 2].concat(STK[--TOP]);
                  stack[top - 2] = stack[top - 2].concat(stack[--top]);
                  continue;

               case Laye.OP_INFIX: // [-1] INFIX(A) STK[TOP - 2] = STK[TOP - 2].infixOp(K[A], STK[--TOP]);
               ...
               case Laye.OP_CALL: // []
               {
                  ...
                  switch (target.valueType)
                  {
                     case FUNCTION:
                     {
                        if (target instanceof LayeFunction)
                        {
                           final LayeFunction func = ((LayeFunction) target);
                           if (func.proto.generator)
                           {
                              throw new LayeRuntimeException("<unknown>", proto.hasLineInfos ? proto.lineInfo[pc] : 0, "cannot call a generator function. use 'resume' instead.");
                           }
                           stack[top++] = func.call(argList);
                        }
                        else if (target instanceof LayeJavaFunction)
                        {
                           stack[top++] = ((LayeJavaFunction) target).call(argList);
                        }
                        continue;
                     }
                     default:
                        throw new IllegalArgumentException("could not call type " + target.typeof().asstring());
                  }
                   */
               ...
               case Laye.OP_HALT: // Halt is used to return the last value or null
                  returned = true;
                  pc = 0;
                  if ((i >>> Laye.POS_A) == 1)
                  {
               ...
               case Laye.OP_THIS: // [1]
                  ...
                  // Should ONLY be allowed for function calls, error on anything else
               case Laye.OP_BASE: // [1]
               ...
               case Laye.OP_DEREF: // [0]
                  ...
                  {
                     throw new LayeRuntimeException("<unknown>", proto.hasLineInfos ? proto.lineInfo[pc] : 0, "Can only dereference references to variables.");
                  }
               ...
               case Laye.OP_LIST: // [A, 1 - A] LIST(A) (STK[TOP - A - 1], STK[TOP - A], ..., STK[TOP - 1])
                  if ((a = i >>> Laye.POS_A) == 0)
                  {
               ...
               case Laye.OP_FOR_PREP: // [0] FOR_PREP(A, B)
               {
                  ...
                  //System.out.println("FOR PREP " + locals[b] + ", " + limit + ", " + step);
                  continue;
               }
