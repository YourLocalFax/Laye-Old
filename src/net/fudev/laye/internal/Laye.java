...
public final class Laye
{
    ...
    *  <pre>[0] CLOSE(A)<br/>for(i = openUps.len(); --i >= A;) openUps[i].close();</pre> */
   public static final int OP_CLOSE = 0x00;
   /** Pops A values from the stack.
    *  <pre>[-A] POP(A)<br/>TOP -= A; */
   public static final int OP_POP = 0x01;
   /** Duplicates the value at (TOP - A - 1) and puts it on the top of the stack.
    *  <pre>[1] DUP(A)<br/>STK[TOP++] = STK[TOP - A - 1];</pre> */
   public static final int OP_DUP = 0x02;

   /** Push the root table on to the stack.
    *  <pre>[1] LOAD_ROOT()<br/>STK[TOP++] = ROOT;</pre> */
   public static final int OP_LOAD_ROOT = 0x03;
   /** Loads local variable A and pushes it onto the stack.
    *  <pre>[1] LOAD(A)<br/>STK[TOP++] = LOC[A];</pre> */
   public static final int OP_LOAD = 0x04;
   /** Stores the top value in the stack in local variable A.
    *  <pre>[0] STORE(A)<br/>LOC[A] = STK[TOP - 1];</pre> */
   public static final int OP_STORE = 0x05;
   /** Creates a new slot in STK[TOP - 3] with the key STK[TOP - 2] and value STK[TOP - 1].
    *  If A is not zero, the slot is made constant.
    *  <pre>[-2] NEW_SLOT(A)<br/>STK[TOP - 3].newSlot(STK[TOP - 2], STK[TOP - 1]);<br/>STK[TOP - 3] = STK[TOP - 1];</pre> */
   public static final int OP_NEW_SLOT = 0x06;
   /** Deletes a slot in STK[top - 2] with the key STK[TOP - 1].
    *  The value that was previously stored in that slot is pushed to the stack.
    *  If no slot existed, null is pushed instead.
    *  <pre>[-1] DEL_SLOT()<br/>STK[TOP - 2] = STK[TOP - 2].delSlot(STK[--TOP]);</pre> */
   public static final int OP_DEL_SLOT = 0x07;
   /** Loads up value A and pushes it onto the stack.
    *  <pre>[1] GET_UP(A)<br/>STK[TOP++] = UPS[A];</pre> */
   public static final int OP_GET_UP = 0x08;
   /** Stores the top value in the stack in up value A.
    *  <pre>[0] SET_UP(A)<br/>UPS[A] = STK[TOP - 1];</pre> */
   public static final int OP_SET_UP = 0x09;
   /** Gets a value at the key STK[TOP - 1] from value STK[TOP - 2] and pushes it to the stack.
    * <pre>[-1] GET_INDEX()<br/>STK[TOP - 2] = STK[TOP - 2].get(STK[TOP - 1])</pre>*/
   public static final int OP_GET_INDEX = 0x0A;
   /** Sets a value at they key STK[TOP - 2] from the value STK[TOP - 3] to STK[TOP - 1] and leaves that value on the stack.
    *  <pre>[-2] SET_INDEX()<br/>STK[TOP - 3].set(STK[TOP - 2], STK[TOP - 1]);<br/>STK[TOP - 3] = STK[TOP - 1];<br/>TOP -= 2;</pre>*/
   public static final int OP_SET_INDEX = 0x0B;
   /** Loads a constant from the constant pool and pushes it onto the stack.
    *  <pre>[1] LOAD_CONST(A)<br/>STK[TOP++] = K[A];</pre>*/
    ...
    *  <pre>[1] LOAD_BOOL(A)<br/>STK[TOP++] = A != 0;</pre> */
    ...
    *  <pre>[1 - A] OP_LIST(A)<br/>(STK[TOP - A - 1], STK[TOP - A], ..., STK[TOP - 1]);</pre> */
